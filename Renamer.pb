...
If OpenWindow(#Win_Main, 0, 0, 540, 390, #Programmname,  #PB_Window_SystemMenu|#PB_Window_MinimizeGadget|#PB_Window_TitleBar|#PB_Window_ScreenCentered|#PB_Window_Invisible)
    ...
    FrameGadget(#F3D_Nummerierung, 310, 40, 230, 55, "Nummerierung", #PB_Frame_Single)
    ...
    FrameGadget(#F3D_ReplaceText, 310, 95, 230, 140, "Dateiname bearbeiten", #PB_Frame_Single)
    ...
    FrameGadget(#F3D_Typ, 0, 320, 240, 70, "Methode", #PB_Frame_Single)
    ...
    FrameGadget(#F3D_EditDate, 310, 235, 230, 85, "Datum ändern", #PB_Frame_Single)
    ...
    FrameGadget(#F3D_EditAttribute, 310, 320, 230, 70, "Attribute ändern", #PB_Frame_Single)
    ...
    FrameGadget(#F3D_Filearea, 0, 5, 305, 315, "", #PB_Frame_Single)
...
Declare GetSelectetListAmount(GadgetID)
...
Procedure GetSelectetListAmount(GadgetID)
    Amount.l = 0
...
Procedure RefreshFileList()
    ClearGadgetItems(#LstVw_Filelist)
    ForEach FileList()
        ...
        EndIf
    Next
    SetGadgetText(#Txt_Filelist, Str(ListSize(FileList())) + " Dateien")
EndProcedure
    ...
    ForEach FileList()
        ...
        If GetGadgetState(#Rad_Typ_Copy) = #True
            ...
            If GetGadgetState(#ChkBox_OverwriteMsg) = #False And FileSize(SaveFolder + Entr + "." + Exts) > -1
                If MessageRequester(#Programmname, "Die Datei " + SaveFolder + Entr + "." + Exts + " existiert bereits," + #nL + "soll Sie überschrieben werden?", #PB_MessageRequester_YesNo) = #PB_MessageRequester_Yes
...
Procedure CheckPattern(String.s)
        ...
        If Event = #PB_Event_Gadget
            ...
            Select EventGadget()
                Case #Btn_MusterInfo
                    ...
                    MessageRequester(#Programmname, String$, #PB_MessageRequester_Ok|#PB_MessageRequester_Info)
                    
                Case #Btn_Start
                    If CheckFilename(GetGadgetText(#Stg_Name)) = #False
                        If MessageRequester(#Programmname, "Ein Dateiname darf keines der folgenden Zeichen enthalten:" + #nL + #nL + "\ / : * ? " + Chr(34) + " < > |" + #nL + "" + #nL + "Ungültige Zeichen werden automatisch entfernt." + #nL + "Möchten Sie fortfahren?", #PB_MessageRequester_YesNo|#PB_MessageRequester_Error) = #PB_MessageRequester_Yes
                            StartRenameEvent()
                        EndIf
                    Else
                        StartRenameEvent()
                ...
                Case #Btn_Info
                    MessageRequester("Informationen", #Programmname + " " + #Programmvers + #nL + #nL + "Dieses Programm ist Freeware und darf somit" + #nL + "kostenlos verwendet werden." + #nL + #nL + "Copyright©Kai Gartenschläger, 2006" + #nL, #PB_MessageRequester_Ok|#PB_MessageRequester_Info)
                ...
                Case #Btn_Help
                    ...
                    ;Else
                    MessageRequester("Fehler", "Die Hilfedatei konnte nicht gefunden werden.", #PB_MessageRequester_Ok|#PB_MessageRequester_Info)
                    ;EndIf
                ...
                Case #Btn_SaveLog
                    ;SavePath$ = SaveFileRequester("Log Datei speichern unter", "", "Textdatei (*.TXT)|*.TXT|Alle Dateien (*.*)|*.*", 0)
                    ;If SavePath$ <> ""
...
CatchError:
Msg$ = "Ein Fehler ist in Zeile " + Str(ErrorLine()) + " aufgetreten:" + #nL + #nL
Msg$ + "Beschreibung: " + ErrorMessage() + #nL + #nL
Msg$ + "Fehlerereignisse: " + Str(ErrorCode()) + #nL + #nL
...
If MessageRequester(#Programmname, Msg$, #PB_MessageRequester_Yes|#PB_MessageRequester_Error) = #PB_MessageRequester_Yes
